...
function composer(){screenEl.innerHTML='<form class="composer"><button type="button" class="emoji" aria-label="Emoji">☺</button><input id="messageInput" autocomplete="off" placeholder="Type a message"><button type="button" class="clip" aria-label="Attach">⌕</button><button type="submit" class="send" aria-label="Send">➤</button></form>';const form=screenEl.querySelector('form');const sendMessage=e=>{e.preventDefault();const input=form.querySelector('input');const text=input.value.trim();if(!text)return;bubble(text,'outgoing');askLanguage();input.value='';};form.addEventListener('submit',sendMessage);form.querySelector('.send').addEventListener('click',sendMessage);screenEl.querySelector('input').focus();}
