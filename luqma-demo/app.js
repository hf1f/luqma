...
function composer(){screenEl.innerHTML='<form class="composer" onsubmit="return false"><button type="button" class="emoji" aria-label="Emoji">☺</button><input id="messageInput" autocomplete="off" placeholder="Type a message"><button type="button" class="clip" aria-label="Attach">⌕</button><button type="button" class="send" aria-label="Send">➤</button></form>';const form=screenEl.querySelector('form');const sendMessage=e=>{e.preventDefault();e.stopPropagation();const input=form.querySelector('input');const text=input.value.trim();if(!text)return;bubble(text,'outgoing');askLanguage();input.value='';};form.addEventListener('submit',e=>{e.preventDefault();e.stopPropagation();});form.querySelector('.send').addEventListener('click',sendMessage);screenEl.querySelector('input').addEventListener('keydown',e=>{if(e.key==='Enter'){e.preventDefault();sendMessage(e);}});screenEl.querySelector('input').focus();}
